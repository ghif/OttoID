...
class LogisticRegression(object):
    ...
    def __init__(self, n_in, n_out):
        ...
        """
        self.x = T.matrix('x')  # the data is presented as rasterized imgaes
    ...
    def negative_log_likelihood(self, y):
        ...
        of this model under a given target distribution. (log-loss)
    ...
    def compile(self,
        ...
        g_W = T.grad(cost=self.negative_log_likelihood(self.y), wrt=self.W)
        g_b = T.grad(cost=self.negative_log_likelihood(self.y), wrt=self.b)
        ...
        self.n_train_batches = train_set_x.eval().shape[0] / batch_size
        self.n_valid_batches = valid_set_x.eval().shape[0] / batch_size

        self.n_test_batches = None


        if test_set_x is not None:
            self.n_test_batches = test_set_x.eval().shape[0] / batch_size

        

    def training_sgd(self,n_epochs=1000):
        ###############
        # TRAIN MODEL #
        ###############
        ...
        while (epoch < n_epochs) and (not done_looping):
            ...
            for minibatch_index in xrange(self.n_train_batches):
                ...
                if (iter + 1) % validation_frequency == 0:
                    ...
                    print(
                        'epoch %i / %s, minibatch %i/%i, validation loss %f %%' %
                        (
                            epoch,
                            n_epochs,
                            minibatch_index + 1,
                ...
                # if patience <= iter or best_validation_loss == 0.0:
